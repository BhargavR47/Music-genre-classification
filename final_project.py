...
from sklearn.metrics import accuracy_score, confusion_matrix #used to get accuracy metrics for models
...
# start creating a table for the data and import into CSV
tbeg = time.time()
...
# for g in genres:
#     for filename in os.listdir(r'C:/Users/jonah/Desktop/Project/Data/genres_original/'+g):
#         songname = r'C:/Users/jonah/Desktop/Project/Data/genres_original/'+ g + '/' + filename
...
# tend = time.time()
# print('total data processing time: %.4f  '% (tend-tbeg))
...
# # plotting the correlation matrix between the variables or features
def plot_corr(data):
    corrMatrix = data.corr()
    plt.figure(figsize = (16,13)),	plt.title("Correlation of Features")
    sn.set(font_scale=1)
    corr_heat = sn.heatmap(corrMatrix, annot=True,cmap="YlGnBu")
    bot, TOP = corr_heat.get_ylim()
    corr_heat.set_ylim(bot+0.5, TOP-0.5)
    plt.show()
plot_corr(data)
...
def plot_matrix(cm, title, genre):
  ...
  sn.set(font_scale=3)
  corrected = sn.heatmap(df_cm, annot=True, cmap="YlGnBu") # have to add in the limits in order to correct for matplotlib and seaborn version 
...
# sn.set(font_scale=3)
# corrected = sn.heatmap(grid_cm, annot=True, cmap="YlGnBu") # have to add in the limits in order to correct for matplotlib and seaborn version 
# bottom, top = corrected.get_ylim()
# corrected.set_ylim(bottom+0.5, top-0.5)
...
#     # best_n_est=2+ran_forest.index(max(ran_forest))  UNCOMMENT
    ...
    # plt.plot(numpy.arange(2,20),ran_forest) # this plots the accuracy vs. n_estimators
    ...
    print("Training set accuracy: {:.3f}".format(rf_random.score(X_train,y_train)))
...
# Training model using MLP
from sklearn.neural_network import MLPClassifier
def multilayer_perc(X_train, y_train, X_test, y_test, genre):
    tbeg = time.time()
    clf = MLPClassifier(solver='lbfgs', alpha=1e-5, hidden_layer_sizes=(20,10), early_stopping=(True), learning_rate='adaptive', random_state=2, max_iter =2000)
    ...
    print("Test set accuracy: {:.2f}".format(accuracy_score(y_test,MLP_predict)))
...
def naive_bayes(X_train,y_train, X_test,y_test, genre):
    ...
    print("Training set accuracy for Gaussian: {:.3f}".format(gnb.score(X_train,y_train)))
...
def decisionTree(X_train, y_train, X_test, y_test, genre):
    ...
    print("Train set accuracy: {:.2f}".format(tree_model.score(X_train, y_train)))
...
def NN_solve(X, X_train, y_train, X_test, y_test, genre):
    ...
    model.summary()
    # train model
    history = model.fit(X_trainn, y_trainn, validation_data=(X_testt, y_testt), batch_size=32, epochs=300)
